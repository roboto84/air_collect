...
class Air:
    def __init__(self, unit_standard: str = 'metric', weather_data: dict = None, date: str = 'n/a') -> NoReturn:
        ...
        if weather_data:
            for key in weather_data:
                self.set_weather_attribute(key, weather_data[key])

    def set_weather_attribute(self, attribute_type: str, attribute_value: Any) -> NoReturn:
    ...
    def set_weather_with_array(self, data_array: List) -> NoReturn:
        ...
        for index in range(len(data_array)):
            if data_array[index] != 'n/a':
                self.set_weather_attribute(weather_keys[index], data_array[index])

    def get_all_weather(self) -> dict:
        return self.weather

    def get_basic_weather(self) -> dict:
        return self.get_sub_dict(['temperature', 'temperatureApparent', 'moonPhase', 'humidity', 'dewPoint',
                                  'weatherCode', 'precipitationProbability', 'precipitationType',
                                  'pressureSurfaceLevel'])

    def get_pollution(self) -> dict:
        return self.get_sub_dict(['epaIndex', 'epaHealthConcern', 'epaPrimaryPollutant', 'particulateMatter10',
                                  'particulateMatter25', 'pollutantCO', 'pollutantNO2', 'pollutantO3', 'pollutantSO2'])

    def get_pollen(self) -> dict:
        return self.get_sub_dict(['grassIndex', 'treeIndex', 'weedIndex'])

    def get_date(self) -> str:
        return self.weather['date']

    def get_sub_dict(self, key_array) -> dict:
        return {k: self.weather[k] for k in self.weather.keys() & key_array}

    def __str__(self) -> str:
        return f'{self.air_to_string("summary", self.weather)}'

    def data_to_csv_string(self) -> str:
        data_string = f'{self.air_to_string("csv", self.weather)}'
        return data_string
    ...
    def air_to_string(self, output_type: str, data: dict) -> str:
        data_as_string = ''
        for key in data:
