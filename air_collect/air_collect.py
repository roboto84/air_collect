...
class AirCollect:
    def __init__(self, logging_object: Any, api_key: str, location_lat_long: List[float],
        ...
        self._next_days_data_file: str = FileHandler.get_full_file_path(file_settings['next_days']['data_file'])

        self._api_key = api_key
        self._lat_long = location_lat_long
    ...
    def run_metronome(self) -> None:
        try:
            ...
            scheduler.add_job(self.get_current_data,
                              'interval',
                              start_date=start_time,
                              seconds=self._query_api_interval)
            scheduler.add_job(self.get_daily_data,
                              'cron',
                              day_of_week='*',
                              hour=0,
                              minute='5')
    ...
    def get_current_data(self) -> None:
        data: dict = DataSource.query_climate_cell_api_timelines(self._logger,
                                                                 self._api_key,
                                                                 self._lat_long,
        ...
        if data:
            ...
            FileHandler.write_data_append(f'{air_obj.data_to_csv_string()}', self._live_data_file)

    def get_daily_data(self) -> None:
        ...
        if data:
            open(self._next_days_data_file, 'w').close()
            FileHandler.write_data_append(Air(UNITS).data_key_order(), self._next_days_data_file)
            for day in data:
                air_obj = Air(UNITS, day['values'], day['startTime'])
                self._logger.info(f'{air_obj}')
                FileHandler.write_data_append(f'{air_obj.data_to_csv_string()}', self._next_days_data_file)
